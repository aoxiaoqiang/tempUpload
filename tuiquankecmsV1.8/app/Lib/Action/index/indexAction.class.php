...
$appkey=C('yh_taobao_appkey');
		...
		$apppid=C('yh_taobao_pid');
		$apppid=explode('_', $apppid);
		$AdzoneId=$apppid[3];
if(!empty($appkey) && !empty($appsecret) && $k && $count<10 && !empty($AdzoneId)){       
$c = new TopClient;
$c->appkey = $appkey;
...
$items[$k+$count]['quan']=$arr[0][1];
$items[$k+$count]['coupon_click_url']=$v['coupon_click_url'];
$items[$k+$count]['num_iid']=$v['num_iid'];
$items[$k+$count]['title']=$v['title'];
$items[$k+$count]['coupon_price']=$v['zk_final_price']-$items[$k+$count]['quan'];
if($v[$k+$count]['user_type']==1){
$items[$k+$count]['shop_type']='B';	
}else{
$items[$k+$count]['shop_type']='C';	
}
$items[$k+$count]['price']=$v['zk_final_price'];
$items[$k+$count]['volume']=$v['volume'];
$items[$k+$count]['pic_url']=$v['pict_url'];

}
// $this->assign('taobao', $item);
