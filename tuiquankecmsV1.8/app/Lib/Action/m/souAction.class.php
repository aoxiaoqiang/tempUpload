<?php
class souAction extends FirstendAction
{
...
$items_list = $this->_mod->where($where)->field('title,cate_id,add_time,id,pic,info')->order($order)->page($page)->limit($size)->select();	
if($items_list){
$goodslist=array();
...
$goodslist[$k]['info']=cut_html_str($v['info'],80);	
