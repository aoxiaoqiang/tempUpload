...
$orderid = $this->_param('orderid','trim');	
$exist = M('order')->where(array(
            'orderid' => $orderid
        ))->count('id');
        
if ($exist) {
$this->ajaxReturn(0, '此订单号已经存在，请勿重复提交');
}

$data['uid'] = $this->visitor->get('id');
...
$this->ajaxReturn(0, $this->error);
}

return $this->ajaxReturn(1,'提交成功！请15分钟后再来查看积分信息');
...
public function index() {
	...
	public function modify() {
		if(IS_POST){
			...
			$password2 = $this->_param('password2','trim');
			$data=array(
				'nickname'=>$this->_param('nickname','trim'),
//				'username'=>$this->_param('username','trim'),
				'qq'=>$this->_param('qq','trim'),
				'wechat'=>$this->_param('wechat','trim'),
			);
			if($this->_param('avatar','trim')){
				$data['avatar'] = $this->_param('avatar','trim');
			}
			if($password){
	...
	public function tixian() {
		if(IS_POST){
			$F=M('balance');
			$mymoney = $this->_param('money','trim');
     		...
     		if($mymoney>$balance['money']){
				...
				exit();
     			...
     			$data=array(
					...
					'name'=>$this->_param('name','trim'),
					'method'=>$this->_param('method','trim'),
					'allpay'=>$this->_param('allpay','trim'),
					'status'=>0,
					'content'=>$this->_param('content','trim'),
					'create_time'=>time()
				);
			$res = $F->add($data);
			if($res !== false){
